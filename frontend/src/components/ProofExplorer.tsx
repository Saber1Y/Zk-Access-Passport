...
export default function ProofExplorer() {
  ...
  const isReplay =
    lastError?.toLowerCase().includes("replay") ||
    lastError?.toLowerCase().includes("nullifier") ||
    false;
  ...
  function getEvents() {
    ...
    if (lastTxHash) {
      events.push({
        label: "Proof transaction submitted",
        done: true,
        failed: false,
      });
    }
    ...
    if (status === "verified") {
      events.push({
        label: "Proof accepted by contract",
        done: true,
        failed: false,
      });
      events.push({
        label: "Nullifier stored on-chain",
        done: true,
        failed: false,
      });
    }
    ...
    if (status === "rejected" && lastError) {
      if (isReplay) {
        events.push({
          label: "Replay attempt rejected — nullifier already spent",
          done: false,
          failed: true,
        });
      } else {
        events.push({
          label: "Proof rejected by contract",
          done: false,
          failed: true,
        });
      }
  ...
  return (
    <div>
      ...
      <p style={{ color: "#666", fontSize: "0.85rem", marginBottom: "1rem" }}>
        Real data from the latest proof submission and Soroban contract
        interaction.
      </p>
      ...
      <div className="card">
        ...
        {!proof ? (
          ...
          >
            No proof generated yet. Complete a remittance or RWA investment to
            see proof details here.
          </div>
        ) : (
          <>
            <div
            ...
            >
              <SummaryBox
                label="Use Case"
                value={useCase === 0 ? "Remittance" : "RWA Investment"}
              />
              <SummaryBox
                label="Proof Status"
                value={
                ...
                color={
                  lastError ? "#dc2626" : lastTxHash ? "#059669" : "#6366f1"
                }
              />
              <SummaryBox
                label="Network"
                value={freighter.network || "Unknown"}
              />
              <SummaryBox
                label="Contract ID"
                value={truncate(CONTRACT_ID, 6)}
                mono
              />
            </div>
            ...
            >
              ...
              {nullifierHash && (
                <SummaryBox
                  label="Nullifier"
                  value={truncate(nullifierHash, 8)}
                  mono
                />
              )}
              {submittedAt > 0 && (
                  ...
                  <table style={{ width: "100%", fontSize: "0.78rem" }}>
                    <tbody>
                      {[
                        ...
                        [
                          "Investor Type",
                          credential.restricted ? "Restricted" : "Standard",
                        ],
                      ].map(([label, val]) => (
                        <tr key={label}>
                          <td style={{ color: "#888", padding: "0.3rem 0" }}>
                          ...
                          <td style={{ textAlign: "right", opacity: 0.5 }}>
                            ••••
                          </td>
                        </tr>
                      ))}
                ...
                return (
                  <div className="card" style={{ marginTop: "1rem" }}>
                    ...
                    >
                      ...
                      <a
                        href={stellarTxUrl(lastTxHash)}
                        target="_blank"
                        rel="noopener noreferrer"
                        style={{
                          ...explorerCopyBtnStyle,
                          textDecoration: "none",
                          display: "inline-flex",
                          alignItems: "center",
                          gap: 4,
            ...
            {proof && (
              ...
              >
                ...
                submitted on-chain in this MVP. Remittance release and RWA token
                minting are demo business outcomes, not on-chain actions.
              </p>
            )}
          </>
        )}
