...
export default function Remit() {
  const saved = localStorage.getItem('zk_access_credential')
  ...
  async function handleProve() {
    ...
    setResult(null)
    try {
      const proof = await generateProof(0, {
        age: cred.age, kyc_level: cred.kyc_level,
        already_sent: cred.already_sent, monthly_limit: cred.monthly_limit,
        credential_secret: cred.credential_secret,
        required_age: requiredAge, required_kyc: requiredKyc, amount,
      })
      setResult(proof)
    } catch (e) {
      setError(e.message)
  ...
  return (
    <div>
      <div className="card">
        <h2 style={{ marginBottom: '0.5rem' }}>ZK Remit Pass</h2>
        <p style={{ color: '#666', marginBottom: '1rem', fontSize: '0.9rem' }}>
          Prove you are eligible to send a remittance without revealing your age,
          KYC status, or bank balance.
        </p>
      ...
      {result && (
        <div className="card">
          ...
          <p className="success" style={{ marginBottom: '0.75rem' }}>
            Your credentials satisfy the requirements. No private data was revealed.
          </p>
          ...
          <table style={{ width: '100%', fontSize: '0.85rem', marginBottom: '0.75rem' }}>
            ...
            <tbody>
              {result.publicInputs.map((pi, i) => (
                <tr key={i}>
                  <td style={{ color: '#666' }}>{['Age ≥', 'KYC ≥', 'Amount ($)', 'Nullifier'][i]}</td>
